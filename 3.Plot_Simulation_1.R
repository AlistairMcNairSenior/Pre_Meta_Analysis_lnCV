...
# This script creates a figure for the complete set of all simlulation parameters explored in the first simulation.

# Clean up the R Environment 
rm(list=ls())

# Set the working directory
wd<-"/Users/alistairsenior/Library/CloudStorage/OneDrive-TheUniversityofSydney(Staff)/Step1_lnCV/Pre_Meta_Analysis_Github"
setwd(wd)
...
# Load the results
load("Aggregate_Data/agg_results_simulation_1.Rdata")
head(agg_results)
...
for(h in 1:length(tau2s)){
	...
	# Create an output file for plotting
	y_counter<-y_counter+1
	file_name<-paste0("simulation_1_", tau2s[h], ".pdf")
...
# Does the heterogeneity estimated by lnCV pre-analysis predict bias in tau2 of lnRR

# Load the results
load("Aggregate_Data/long_lnCV_simulation_1.Rdata")
load("Aggregate_Data/long_lnRR_simulation_1.Rdata")
head(lnCV_long)
head(lnRR_long)
lnRR_long<-as.data.frame(lnRR_long)
lnCV_long<-as.data.frame(lnCV_long)

# Subset
tag<-which(lnRR_long$tau2 == 9e-04)
lnRR_long<-lnRR_long[tag,]
lnCV_long<-lnCV_long[tag,]

binary_bias<-(abs(lnRR_long$MA_Tau2 - lnRR_long$tau2) / abs(lnRR_long$conv_Tau2 - lnRR_long$tau2)) > (1 + 1/3) 
Q_test<-(lnCV_long$lnCV1_Q_p < 0.05) + (lnCV_long$lnCV2_Q_p < 0.05)

bias_data<-as.data.frame(cbind(binary_bias, Q_test))

# Proportion of cases where conventional outperforms, as a function of Q-tests in lnCV
ddply(bias_data, .(Q_test), summarise, mean(binary_bias))

# How sever can this bias be
...
# Subset
tag<-which(lnRR_long$tau2 == 9e-02)
lnRR_long<-lnRR_long[tag,]
lnCV_long<-lnCV_long[tag,]
