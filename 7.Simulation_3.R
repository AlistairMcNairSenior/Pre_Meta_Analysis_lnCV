...
template<-data.frame(conv_ests=rep(NA, n_reps), conv_SE=NA, conv_Tau2=NA, conv_ICC=NA, wght_ests=NA, wght_SE=NA, wght_Tau2=NA, wght_ICC=NA, lnCV1_Tau2=NA, lnCV1_Q=NA, lnCV1_Q_p=NA, lnCV1_df=NA, lnCV2_Tau2=NA, lnCV2_Q=NA, lnCV2_Q_p=NA, lnCV2_df=NA, MA_ests=NA, MA_SE=NA, MA_Tau2=NA, MA_ICC=NA)
...
for(p in 1:nrow(parameters)){
	...
	for(n in 1:n_reps){
		...
		while(sum(is.na(template_p[n,])) > 0){
			...
			conv_model<-try(rma.mv(yi = yi, V = vi, random=list(~1|Study, ~1|Effect), data=data_drop), silent=T)
			...
			if(class(conv_model)[1] == "rma.mv"){
				...
				template_p$conv_Tau2[n]<-sum(conv_model$sigma2[c(1:2)])
				template_p$conv_ICC[n]<-conv_model$sigma2[1] / sum(conv_model$sigma2[c(1:2)])
			}
			...
			# Fit the model
			wght_model<-try(rma.mv(yi = yi, V = vi_2, random=list(~1|Study, ~1|Effect), data=data_n), silent=T)
			
			# Save the results if the model fitted
			if(class(wght_model)[1] == "rma.mv"){
				template_p$wght_ests[n]<-wght_model$b
				template_p$wght_SE[n]<-wght_model$se
				template_p$wght_Tau2[n]<-sum(wght_model$sigma2[c(1:2)])
				template_p$wght_ICC[n]<-wght_model$sigma2[1] / sum(wght_model$sigma2[c(1:2)])
			}
			...
			lnCV1_model<-try(rma.mv(yi = yi, V = vi, random=list(~1|Study, ~1|Effect), data=data_lnCV1), silent=T)
			lnCV2_model<-try(rma.mv(yi = yi, V = vi, random=list(~1|Study, ~1|Effect), data=data_lnCV2), silent=T)			
			...
			if(class(lnCV1_model)[1] == "rma.mv" & class(lnCV2_model)[1] == "rma.mv"){
				...
				template_p$lnCV1_Tau2[n]<-sum(lnCV1_model$sigma2)[1]
				...
				template_p$lnCV2_Tau2[n]<-sum(lnCV2_model$sigma2)[1]
				...
				mcv1<-exp(lnCV1_model$b + 0.5 * sum(lnCV1_model$sigma2[c(1:2)]))[1]
				mcv2<-exp(lnCV2_model$b + 0.5 * sum(lnCV2_model$sigma2[c(1:2)]))[1]
				data_n$vi_3<-mcv1^2/data_n$Treatment.n + mcv2^2/data_n$Control.n + mcv1^4/(2 * data_n$Treatment.n^2) + mcv2^4/(2 * data_n$Control.n^2)
				
				# If dropping data get simple lnRR
				if(parameters_p$proportion_lost > 0){
					data_n$yi[drop]<-log(data_n$Treatment.Mean[drop] / data_n$Control.Mean[drop])
				}
				...
				MA_model<-try(rma.mv(yi = yi, V = vi_3, random=list(~1|Study, ~1|Effect), data=data_n), silent=T)

				# Save the results if the model fitted
				if(class(MA_model)[1] == "rma.mv"){
					template_p$MA_ests[n]<-MA_model$b
					template_p$MA_SE[n]<-MA_model$se
					template_p$MA_Tau2[n]<-sum(MA_model$sigma2[c(1:2)])
					template_p$MA_ICC[n]<-MA_model$sigma2[1] / sum(MA_model$sigma2[c(1:2)])
				}
