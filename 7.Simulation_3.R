...
# Note the script below runs just 100 replicates of each parameter set - see (L72)

# The full simulation described in test is based the results for 100k replicates, generated by running this script across 1000 instances on a high performance computer (HPC). The commented code on L12 takes the input data from the HPC noting which instance is being run to save the data from this instance. Running it across a HPC in this way will generate 1000 sets of results. Output from this script (and multiple instances thereof) can be processed in the next script '8.Aggregate_Simulation_3.R, on a local machine.

# Clean up the R Environment 
rm(list=ls())

# If running on a HPC take incoming arguments from bash, by using L12 and commenting out L13
#index<-commandArgs()[6] 
index<-1

# Where are we working
directory<-"/Users/alistairsenior/Library/CloudStorage/OneDrive-TheUniversityofSydney(Staff)/Step1_lnCV/Pre_Meta_Analysis_Github"
setwd(directory)
...
# If we are on the first instance create the output directory
if(index == 1){
	if(dir.exists("Output_Simulation_3")){
		dir_delete("Output_Simulation_3")
	}
	dir.create("Output_Simulation_3")	
}
...
# Number of reps for this array of the simulation
n_reps<-100
...
save(results, file=paste0("Output_Simulation_3/Simulation_3_", index, ".Rdata"))
save(parameters, file="Output_Simulation_3/Parameters_3.Rdata")
